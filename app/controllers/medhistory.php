...
$msg="";
$msg_class="";
$medhis=mysqli_query($conn,"select * from medicalhist where pnt_id='".$_SESSION['p_id']."'");
$cnt=1;

if (isset($_GET['cancel'])){

    $update="delete from medicalhist where pnt_id='".$_SESSION['p_id']."' and id='".$_GET['id']."'";
...
if (isset($_POST['addmedhist'])){
    ...
    if (empty($_POST['p_medhist']) ||empty($_POST['med_hist'])|| empty($_POST['famhist']|| empty($_POST['trehist']))){
        ...
        $medhistory="insert into medicalhist set pnt_med_hist='$pnthist',medication_hist='$medhist',family_med_hist='$famhist',treatment_hist='$trehist',pnt_id='".$_SESSION['p_id']."'";
        if (mysqli_query($conn,$medhistory)){
